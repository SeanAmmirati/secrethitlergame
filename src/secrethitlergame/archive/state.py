...
class SecretHitlerState():
    ...
    def after_successful_gov(self, played_card):
        self.enact(played_card)
    ...
    def determine_course_of_action(self, played_card=None):
        last_vote = self.vote_results[-1]

        if last_vote['result'] == 'Passed':
            self.after_successful_gov(played_card)
        else:
